...
from utils.data_utils import cuda_unsqueeze, load_FS_latent
from utils.image_utils import dilate_erosion_mask_path, dilate_erosion_mask_tensor, load_image

toPIL = torchvision.transforms.ToPILImage()
torch.backends.cudnn.benchmark = True
# # Blending.py 1.复制Fk(align) 组合结构向量latent_3
# # Blending.py 2.对齐图像的外观的混合，通过mask外观的loss寻找外观编码的混合权重latent_mixed
class Blending(object):
    ...
    # # 步骤4:Blending.py 复制Fk(align) 组合结构向量 latent_3
    def blend_images(self, img_path1, img_path2, img_path3, sign="realistic"):
        ...
        latent_1, latent_F_mixed = load_FS_latent(
            os.path.join(
                output_dir, "Align_{}".format(sign), "{}_{}.npz".format(im_name_1, im_name_3)
            ),
            device,
        )
        ...
        with torch.no_grad():
            ...
            HM_X = torch.where(
                current_mask == 10, torch.ones_like(current_mask), torch.zeros_like(current_mask)
            )
        ...
        _, latent_F_mixed = load_FS_latent(
            os.path.join(
                output_dir, "Align_{}".format(sign), "{}_{}.npz".format(im_name_1, im_name_2)
            ),
            device,
        )
        I_G, _ = self.net.generator(
    ...
    def save_blend_results(
        ...
        output_image_path = os.path.join(
            self.opts.output_dir, "{}_{}_{}_{}.png".format(im_name_1, im_name_2, im_name_3, sign)
        )
