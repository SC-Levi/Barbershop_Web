...
from utils.data_utils import convert_npy_code, cuda_unsqueeze, load_FS_latent
from utils.image_utils import dilate_erosion_mask_tensor
from utils.seg_utils import save_vis_mask

toPIL = torchvision.transforms.ToPILImage()
torch.backends.cudnn.benchmark = True
# # Alignment.py 1.分割参考图像 并自动生成目标分割
# # Alignment.py 2.嵌入参考图像 找到潜编码latent_1
# # Alignment.py 3.查找与目标分割目标图像 匹配同时与图像相似的潜编码latent_F_mixed
...
class Alignment(object):
    ...
    def create_target_segmentation_mask(self, img_path1, img_path2, sign, save_intermediate=True):
        ...
        gener_mask = torch.where(
            seg_target1 == 0, torch.zeros_like(seg_target1), torch.ones_like(seg_target1)
        )

        hair_mask1 = torch.where(
            seg_target1 == 10, torch.ones_like(seg_target1), torch.zeros_like(seg_target1)
        )
        ...
        gener_mask = torch.where(seg_target2 == 10, torch.ones_like(seg_target2), gener_mask)

        hair_mask2 = torch.where(
            seg_target2 == 10, torch.ones_like(seg_target2), torch.zeros_like(seg_target2)
        )
        seg_target2 = seg_target2[0].byte().cpu().detach()

        ob_region = torch.where(
            (seg_target2 != 10) * (seg_target2 != 0) * (seg_target2 != 15) * (seg_target1 == 0),
        ...
        inpainting_region = torch.where(
            (new_target != 0) * (new_target != 10), 255 * torch.ones_like(new_target), ob_region
        ).numpy()
        tmp = (
            torch.where(
                torch.from_numpy(inpainting_region) == 255, torch.zeros_like(new_target), new_target
            )
            / 10
        )
        new_target_inpainted = (
            cv2.inpaint(tmp.clone().numpy(), inpainting_region, 3, cv2.INPAINT_NS).astype(np.uint8)
            * 10
        )
        ...
        target_mask = new_target_final.to(device).unsqueeze(0).long()
        ...
        for step in pbar:
            ...
            if sign == "realistic":
                ...
                ce_loss += self.loss_builder.cross_entropy_loss_only_background(down_seg, gener_mask)
            else:
        ...
        target_mask = torch.where(
            ob_region.to(device).unsqueeze(0), torch.zeros_like(target_mask), target_mask
        )
        optimizer_align, latent_align = self.setup_align_optimizer()
        latent_end = latent_align[:, 6:, :].clone().detach()
        ...
        if save_intermediate:
            save_vis_mask(
                img_path1, img_path2, sign, self.opts.output_dir, target_mask.squeeze().cpu()
            )

        hair_mask_target = torch.where(
            target_mask == 10, torch.ones_like(target_mask), torch.zeros_like(target_mask)
        )
    ...
    def create_down_seg(self, latent_in):
        gen_im, _ = self.net.generator(
            [latent_in], input_is_latent=True, return_latents=False, start_layer=0, end_layer=8
        )
    ...
    def align_images(
        ...
        # 查找与目标分割目标图像 匹配同时与图像相似的潜编码load_FS_latent
        device = self.opts.device
        ...
        (
            ...
            img_path1=img_path1, img_path2=img_path2, sign=sign, save_intermediate=save_intermediate
        )
        ...
        intermediate_align, _ = self.net.generator(
            [latent_in], input_is_latent=True, return_latents=False, start_layer=0, end_layer=3
        )
        ...
        T2 = time.time()
        print('--------Align Step 1:%ss-------------' % ((T2 - T1)))
        # 步骤3:Alignment.py 查找与目标分割目标图像 匹配同时与图像相似的潜编码latent_F_mixed
        ...
        free_mask = F.interpolate(
            (hair_mask2.unsqueeze(0) * hair_mask_target).float(), size=(256, 256), mode="nearest"
        ).cuda()
        # 
        _, free_mask = self.dilate_erosion(free_mask, device, dilate_erosion=smooth)
        # 
        ...
        latent_F_mixed = latent_F_2 + interpolation_low.unsqueeze(0) * (latent_F_mixed - latent_F_2)
    ...
    def save_align_results(
        self, im_name_1, im_name_2, sign, gen_im, latent_in, latent_F, save_intermediate=True
    ):
